...
public class OutScanActivity extends BisnessBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       //查看必出袋明细
        ...
        btn_mustout.setTextColor(getResources().getColor(R.color.TextWhite));
                ...
                switch (msg.what) {
                    case 1://扫描到包号
                        ...
                        if(tagEpcData!=null&&tagEpcData.getTagid()>0)//如果可以读取未报错
                        {
                            ...
                            {
                                addRsinfo("读取到封签:"+tagEpcData.getTagid()+"状态为"+tagEpcData.getLockstuts()+"不可出库",false);
                                mylog.Write("读取到封签:"+tagEpcData.getTagid()+"状态为"+tagEpcData.getLockstuts()+"不可出库");
                              //  Util.playErr();
                                return;
                            }
...
//                            Log.e("kcrx","compareTo="+thisoutScanBusiInfo.getTotalMoney().compareTo(outScanStockPackInfo.getSackMoney()));
    ...
    private boolean checkStockList(TagEpcData tagEpcData) {
        for(cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.outScanStockPackInfo outScanStockPackInfo:outScanCMD.getStockPackInfoList())
        {
