...
import cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.EnterScanCMD;
import cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.ViewCmdInfo;
import cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.paymentSack;
import cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.stackInfo;
import cn.kcrxorg.chengweiepms.businessmodule.datainfo.EnterScanData;
import cn.kcrxorg.chengweiepms.businessmodule.datainfo.enterScanPackInfo;
...
public class NewEnterScanActivity extends BisnessBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
      ...
      //  businessid=getIntent().getStringExtra("businessid");
        ...
        businessid=getIntent().getStringExtra("businessid");
        String cmddata = tr.getCmdById(NewEnterScanActivity.this, businessid);
                ...
                switch (msg.what) {
                    case 1://扫描到包号
                        ...
                        if(tagEpcData!=null&&tagEpcData.getTagid()>0)//如果可以读取未报错
                        {
                            ...
                            {
                                addRsinfo("读取到封签:"+tagEpcData.getTagid()+"状态为"+ LockStutsHelper.getStutsName(tagEpcData.getLockstuts())+"不可入库",false);
                                mylog.Write("读取到封签:"+tagEpcData.getTagid()+"状态为"+ LockStutsHelper.getStutsName(tagEpcData.getLockstuts())+"不可入库");
                                Util.playErr();
                                return;
                            }
                            if(tagEpcData.getLockeEx())
                            {
                                addRsinfo("读取到封签:"+tagEpcData.getTagid()+"状态为异常不可入库",false);
                                mylog.Write("读取到封签:"+tagEpcData.getTagid()+"状态为异常不可入库");
                                Util.playErr();
                                return;
                            }
