...
public class DESHelper {
	...
	public static boolean checkMAC(String payloads,String mkey)
	{
		String mrs=null;
		String pload=payloads.substring(0, payloads.length()-8);
		String mac=payloads.substring(payloads.length()-8);
		try {
			mrs=encryptStr(pload,mkey);
			String mmac=mrs.substring(mrs.length()-16,mrs.length()-8);
			if(mmac.toUpperCase().equals(mac))
			{
				return true;
			}else {
				return false;
