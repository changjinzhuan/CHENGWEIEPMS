...
import cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.EmptyEnterScanCMD;
...
import cn.kcrxorg.chengweiepms.businessmodule.datainfo.EmptyEnterScanData;
...
//        }
        ...
        if (emptyEnterScanCMD.getStackInfoList().length > 0)//如果需要选择库间的话
        {
            if(emptyEnterScanCMD.getStackInfoList()[0].getSstackName()!=null)//如果集合不是空再添加选择
            {
                ...
                for(cn.kcrxorg.chengweiepms.businessmodule.cmdinfo.stackInfo stackInfo:emptyEnterScanCMD.getStackInfoList())
                {
